...
// バックエンド未接続の間の再生用サンプル(engine の fixtures/traces/*.json 相当)。
// Zod で契約を検証してから使う。ステージ5で HTTP から取得する形に置き換える。
...
export const evenATrace: DFATrace = dfaTraceSchema.parse(evenARaw);
export const anbncnTrace: DTMTrace = dtmTraceSchema.parse(anbncnRaw);
